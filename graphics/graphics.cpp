...
const NXColor DBLACK(0, 0, 0);			// pure black, only works if no colorkey
...
#ifdef NSPIRE
	sdl_screen = SDL_SetVideoMode(320, 240, has_colors ? 16 : 8, SDL_SWSURFACE);
#else
...
/*
#ifdef NSPIRE
	sprintf(fname, "./data/Loading.pbm.tns", data_dir);
#else
...
{
	if (dstx > (-1-wd) && dsty > (-1-ht) && dstx < 321 && dsty < 241)
...
{
	if (x_dst > (-1) && y_dst > (-1-height) && x_dst < 321 && y_dst < 241)
...
{
	if ( (x1 > -1 && y1 > -1 && x1 < 321 && y1 < 241) && (x2 > -1 && y2 > -1 && x2 < 321 && y2 < 241))
